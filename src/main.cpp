...
void task_audio_code(void *parameter)
{
  ...
  {
    if (ap.is_active())
    {                                                  // don't run the loop if AP failed to init
      ...
      if (curr_mode.main.id() == MODE_MAIN_AUDIO)
      {
        ...
        if (blend_counter == 0)
        { // if the counter reset to 0 it means we changed modes
...
void task_spotify_code(void *parameter)
{
  ...
  for (;;)
  {
    ...
    if ((WiFi.status() == WL_CONNECTED))
    {
      ...
      if (curr_mode.main.id() != MODE_MAIN_IMAGE)
      { // only run spotify loop if we are not in image download mode; otherwise the https code will
        sp.update();
        Spotify::public_data_t sp_data = sp.get_data();
        if (sp_data.art_changed && sp_data.is_active)
        { // only update art if spotify is active
...
#ifdef SERVO_DEBUG
    if (Serial.available())
    { // if wants manual control, honor it
      char c = Serial.read();
      switch (c)
      {
      case 'f':
