...
bool connect_wifi(bool async)
{
    ...
    if (WiFi.status() != WL_CONNECTED)
    {
        ...
        // WiFi.config(IPADDR_NONE, IPADDR_NONE, IPADDR_NONE, IPADDR_NONE); // without commenting this out, WiFi.localIp() returns 255.255.255.255 :(
...
void print(const char *format, ...)
{
    ...
    if (strlen(format) > (HTTP_MAX_CHARS - 2))
        Serial.println("WARNING! Printed text will be truncated!");
    int ret = vsnprintf(buffer, HTTP_MAX_CHARS, format, args);
    if (ret < 0)
        Serial.println("WARNING! Print encoding failed!");
    Serial.print(buffer);
