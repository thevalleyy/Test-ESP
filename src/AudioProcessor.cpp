...
bool AudioProcessor::_i2s_init()
{
    i2s_config_t i2s_config = {
        ...
        .communication_format = i2s_comm_format_t(I2S_COMM_FORMAT_STAND_I2S), // (I2S_COMM_FORMAT_STAND_I2S | I2S_COMM_FORMAT_I2S_LSB) originally
...
void AudioProcessor::update_volume()
{
    ...
    if (_audio_first_loop)
    { // the first time through the loop, we have no history, so set the avg volume to the curr volume
    ...
    if (_curr_volume > VOL_PEAK_FACTOR * _avg_volume)
    { // we have a peak
...
void AudioProcessor::_postprocess_fft()
{
    ...
    for (int i = 0; i < FFT_SAMPLES / 2; i++)
    {
        ...
        if (_WHITE_NOISE_EQ)
        { // apply white noise eq
            eq_mult = 255.0 / double(FFT_EQ[i]);
            _v_real[i] *= eq_mult;
        }
        if (_A_WEIGHTING_EQ)
        { // apply A-weighting eq
        ...
        { // apply perceptual binning, only maps the lower half of the samples
            if (i < FFT_SAMPLES / 4)
            {
                curr_bin = i;
...
void AudioProcessor::_detect_beat()
{
    ...
    for (int i = _lowest_bass_bin; i <= _highest_bass_bin; i++)
    { // up to and including the highest bin
...
void AudioProcessor::calc_intensity(int length)
{
    ...
    for (int i = 0; i < length; i++)
    {
        ...
        if ((map_val >= _intensity[i]) && (20 * log10(_avg_volume) > VOL_THRESH_DB) && (map_val >= MIN_BRIGHT_UPDATE))
        { // if the FFT value is brighter AND our volume is loud enough that we trust the data AND it is brighter than our min thresh, update
        ...
        if (_intensity[i] < MIN_BRIGHT_FADE)
        { // if less than the min value, floor to zero to reduce flicker
            _intensity[i] = 0;
        }
...
void AudioProcessor::calc_intensity_simple()
{
    ...
    for (int i = 0; i < FFT_SAMPLES / 2; i++)
    {
        ...
        else
        {
            for (int j = 0; j < NUM_AUDIO_BANDS; j++)
            {
                if (i > int(round(_low_bins[j])) && i <= int(round(_high_bins[j])))
                    _fft_bin[j] += _v_real[i];
            }
        }
    }
...
void AudioProcessor::_interpolate_fft(int old_length, int new_length)
{
    ...
    if (new_length > old_length)
    {
        for (int i = 0; i < new_length; i++)
        {
            ...
            if ((floor_index + 1) < old_length)
            { // find the FFT at the next index
